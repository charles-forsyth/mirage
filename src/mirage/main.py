...
def get_audio_duration(file_path: Path) -> float:
    ...
    try:
        ...
        console.print(
            f"[yellow]Warning: Could not determine duration of {file_path}. Defaulting to 60s.[/yellow]"
        )
        return 60.0
...
def cmd_weather(args: argparse.Namespace) -> None:
    ...
    with console.status(
        f"[bold green]Gathering atmospheric data for {location}...[/bold green]",
        spinner="earth",
    ) as status:
        if not silent:
            status.update(
        ...
        if not silent:
            status.update(
                "[bold blue]Synthesizing immersive audio podcast...[/bold blue]"
            )
        run_command(
            f'cat "{context_file}" | {settings.gen_tts_cmd} --podcast --no-play --audio-format MP3 --output-file "{podcast_file}"',
            quiet=silent,
        )

        if not silent:
            status.update(
                "[bold magenta]Dreaming up background visual...[/bold magenta]"
            )
        run_command(
            f'cat "{context_file}" | {settings.lumina_cmd} --opt --output-dir "{output_dir}" -f background_art.png',
            quiet=silent,
        )

        if not image_file.exists():
            console.print(
                "[yellow]Warning: Lumina failed to generate an image. Creating placeholder.[/yellow]"
            )
            run_command(
                f'{settings.convert_cmd} -size 1024x1024 xc:black "{image_file}"',
                quiet=silent,
            )
        ...
        if generate_video:
            if image_file.exists():
                if not silent:
                    status.update(
                        "[bold cyan]Animating scene with Vidius...[/bold cyan]"
                    )
                ...
                console.print(
                    "[yellow]Skipping video generation: No source image.[/yellow]"
                )
    ...
    if not silent:
        console.print(
            Panel(
                f"[bold green]Experience Ready![/bold green]\nOpen: [link=file://{html_file.absolute()}]{html_file}[/link]",
                border_style="green",
            )
        )
...
def cmd_research(args: argparse.Namespace) -> None:
    ...
    if not silent:
        console.print(
            Panel(
                f"[bold green]Starting Mirage: Research Documentary[/bold green]\nTopic: [cyan]{topic}[/cyan]\nOutput: [yellow]{output_dir}[/yellow]",
                title="Mirage",
            )
        )
    ...
    with console.status(
        f"[bold green]Conducting deep research on: {topic}...[/bold green]",
        spinner="dots",
    ) as status:
        if not silent:
            status.update(
        ...
        if not silent:
            status.update(
                "[bold blue]Scripting and recording documentary...[/bold blue]"
            )
        ...
        if result.returncode != 0:
            console.print(
                f"[bold red]Error generating audio:[/bold red] {result.stderr}"
            )
        ...
        if "--- Generated Podcast Script ---" in full_output:
            try:
                ...
                clean_script = re.sub(
                    r"^(Host|Guest|Speaker \w+):\s*", "", raw_script, flags=re.MULTILINE
                ).strip()
    ...
    if not silent:
        console.print(
            Panel(
                f"[bold green]Documentary Ready![/bold green]\nOpen: [link=file://{html_file.absolute()}]{html_file}[/link]",
                border_style="green",
            )
        )
...
def cmd_news_short(args: argparse.Namespace) -> None:
    ...
    if not silent:
        console.print(
            Panel(
                f"[bold green]Starting Mirage: YouTube Short[/bold green]\nTopic: [cyan]{topic}[/cyan]\nOutput: [yellow]{output_dir}[/yellow]",
                title="Mirage",
            )
        )
    ...
    with console.status(
        f"[bold green]Producing News Short for: {topic}...[/bold green]", spinner="dots"
    ) as status:
        # 1. Script & Voice
        if not silent:
        ...
        if not image_file.exists():
            console.print(
                "[yellow]Visual generation failed. creating placeholder.[/yellow]"
            )
            run_command(
                f'{settings.convert_cmd} -size 1080x1920 xc:darkblue "{image_file}"',
                quiet=silent,
            )
        ...
        # 5. Assembly (FFmpeg)
        if not silent:
            status.update("[bold white]Assembling final video...[/bold white]")
...
def cmd_character(args: argparse.Namespace) -> None:
    ...
    if action == "list":
        ...
        if dest.exists():
            console.print(
                f"[yellow]Character '{args.name}' already exists. Overwriting...[/yellow]"
            )

        console.print(
            f"[magenta]Generating character '{args.name}' with Lumina...[/magenta]"
        )
        ...
        if dest.exists():
            console.print(
                f"[green]Character '{args.name}' created successfully.[/green]"
            )
        else:
...
def cmd_story(args: argparse.Namespace) -> None:
    ...
    with console.status(
        f"[bold green]Weaving story for: {topic}...[/bold green]", spinner="dots"
    ) as status:
        ...
        result = subprocess.run(
            script_gen_cmd, shell=True, capture_output=True, text=True
        )
        ...
        if "--- Generated Podcast Script ---" in full_out:
            try:
                script_text = full_out.split("--- Generated Podcast Script ---")[
                    1
                ].strip()
                script_text = re.sub(
                    r"^(Narrator|Speaker):\s*", "", script_text, flags=re.MULTILINE
                ).strip()
            except Exception:
                pass
        elif "--- Generated Storyteller Script ---" in full_out:
            try:
                script_text = full_out.split("--- Generated Storyteller Script ---")[
                    1
                ].strip()
                script_text = re.sub(
                    r"^(Narrator|Speaker):\s*", "", script_text, flags=re.MULTILINE
        ...
        if character_desc and char_lib_path.exists():
            ...
            if not silent:
                console.print(
                    f"[green]Using character from library: {character_desc}[/green]"
                )
        else:
        ...
        run_command(
            f'{settings.ffmpeg_cmd} -y -f concat -safe 0 -i "{concat_list_file}" -c copy "{merged_video}"',
            quiet=silent,
        )
...
def main() -> None:
    ...
    weather = subparsers.add_parser(
        "weather", help="Generate an Atmospheric Weather Experience"
    )
    weather.add_argument(
        "-l", "--location", default=settings.default_location, help="Location"
    )
    ...
    research = subparsers.add_parser(
        "research", help="Generate a Deep Research Documentary"
    )
    ...
    news = subparsers.add_parser(
        "news-short", help="Generate a YouTube Short News Report"
    )
    ...
    story = subparsers.add_parser(
        "story", help="Generate a 3-part seamless Story Short"
    )
    ...
    char_parser.add_argument(
        "action", choices=["list", "add", "remove", "create"], help="Action"
    )
    char_parser.add_argument(
        "name", nargs="?", help="Character Name"
    )  # Positional name for convenience
    ...
    if hasattr(args, "background") and args.background:
        ...
        with open(settings.log_file, "a") as log_file:
            subprocess.Popen(
                cmd, stdout=log_file, stderr=log_file, start_new_session=True, env=env
            )
